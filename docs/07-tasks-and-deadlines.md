...
A task is a work assignment: it appears as a node on its board and simultaneously in the assignee's My Work, so the person responsible sees it in one place regardless of which board it came from or who created it. This document defines where tasks come from, how they get an assignee, how their due dates are computed, when they surface to the assignee, and how overdue work and reassignment are handled. See 05-flows.md for how a Task step is defined within a flow, and 06-approvals.md for the parallel human-decision path, which shares the same assignment and deadline rules.

## Where Tasks Come From

A task has one of three origins, and the result is the same work item in all three cases:

| Origin | Example |
|---|---|
| **Someone creates it for themselves** | "Call the vendor back on Thursday." |
| **A manager creates it for a direct report** | A team lead assigning follow-up work after a meeting. |
| **A flow step generates it** | The payment-execution task raised once a request clears its final approval. |

This matters because it is what the platform is for: every person opens one place and sees what they need to do, whether it came from a process or from a conversation. A platform that only showed flow-generated work would leave most of a day's real work outside it.

Who may assign to whom: anyone can create a task for themselves; a manager can create one for a direct report (02-tenancy-and-roles.md); and anyone holding the task-assignment capability can create one for any member of the business line.

Because the origins produce the same thing, a task is a node like any other — not a separate kind of record that happens to look like one. It carries the board's fields, sits in a group, has an activity timeline, and is filtered and sorted the same way (03-boards-and-nodes.md). A business line is created with a default space and a **Tasks** board, so a task that belongs to no particular process still has somewhere to live.

## Tasks as Nodes and in My Work

- When a flow run reaches a Task step, the platform creates a task: a node on the board, carrying whatever fields the step defines (title, description, due date, assignee). A task someone creates by hand carries the same fields.
...
A task created by hand is assigned to whoever created it, or to the person it was created for. A task generated by a flow step is assigned by one of four rules, chosen by the Process Designer when the step is configured — the same four an approval uses (see 06-approvals.md):
...
**US-07.5 — Employee: create a task for myself**
As an Employee, I want to create a task for myself, so that everything I need to do lives in one place rather than half in the platform and half in my head.
- Given I am in My Work or on a board, I can create a task with a title, an optional description, a due date, and an assignee.
- When I create it without naming anyone, it is assigned to me and appears in my My Work.
- A task I create with no particular process behind it lands on my business line's default Tasks board.

**US-07.6 — Manager: create a task for a direct report**
As a manager, I want to assign a task to someone who reports to me, so that follow-up work agreed in conversation is tracked like any other work.
- Given a person who reports to me, I can create a task assigned to them, and it appears in their My Work.
- Given a person who does not report to me, assigning to them is only offered if I hold the task-assignment capability (02-tenancy-and-roles.md).
- The task is recorded on its node's activity timeline with me as its creator.
